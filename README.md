...
1. Commit the changes to the `.proto` files.

The Pull Request process for this repository will regenerate the language-specific bindings and visualizations, and commit those in.
Your locally generated bindings are overwritten.

This is because of issues with matching the precise versions.
For example, the go bindings include:
...
If your versions of protoc or protoc-gen-go are different, the generated code
would be different. For consistent control, we only use the versions generated
by CI.

If you want to be sure that the bindings are correct, you can do the following:

1. Make proto file changes
1. Run `make proto` to generate the language-specific bindings and visualizations
1. Check that the bindings work as you expect
1. Commit, push, open a PR (even draft)
1. Wait for CI to run and regenerate the bindings
1. Pull back to your local copy via `git pull origin <branch> --rebase`
1. Check that the bindings are still correct
